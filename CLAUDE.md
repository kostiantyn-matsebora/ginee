...
| D14 | GitHub issues + discussions as task source (added 2026-05-17) — `project-manager` files issues (`@project-manager file bug` / `file feature` using structured templates under `core/templates/issues/`), picks them up (`pick up #<N>`), triages ready issues (`triage`), and promotes discussions to issues (`promote discussion #<N>`); native `open`/`closed` + configurable labels (`engineering-team:ready` / `:in-progress` / `:blocked`) replace `☐`/`☒` glyphs for issue-sourced tasks; PRs auto-close issues via `Closes #N` linkage; tool surface is agnostic (`gh` CLI baseline, GitHub MCP / HTTPS fallback); pickup is always explicit (never auto). **Two repos.** Primary (`github.repo`, inferred from origin) — adopter's own project. Framework upstream (`github.framework-repo`, set at install) — lets adopters file feedback against engineering-team itself via `file framework-bug` / `file framework-feature` / `pick up framework#<N>` / `triage framework` / `promote discussion framework#<N>`. Framework-targeted ops use framework-specific templates (`framework-bug-report.md`, `framework-feature-request.md`) capturing affected framework artefact + version + adapter + locked-decision impact. Working in the framework repo itself (origin == framework) naturally uses framework templates via target-based selection. Full spec `core/github-integration.md` |
