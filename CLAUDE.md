...
| D19 | **Backend coverage floor** (2026-05-19). <ul><li>`backend-engineer` ships **≥ `unit-backend.coverage-threshold` line coverage on changed + added lines** (default `90`, configurable).</li><li>Tests executed + pass before iteration reports complete.</li><li>**Option B** — SA may grant per-task waiver (mechanical / infra-adjacent / baseline-matching), documented in PR; never silent; never retroactive.</li><li>**Functionality-first ordering** — behaviour → error branches → edges → wiring last (smoke-only).</li><li>**Exemptions** — DTOs / pure data; generated code; option-binding classes.</li><li>**No-tooling** → discovery gap to `team-lead`; rule never silently lowers the bar.</li><li>Per-stack runners: `coverlet` / `jest --coverage` / `pytest-cov` / `go test -cover` / `jacoco` / `simplecov` / `cargo-llvm-cov` — see `backend-engineer.details.md`.</li><li>Closes [#29](https://github.com/kostiantyn-matsebora/ginee/issues/29). Migration: `core/MIGRATIONS/D19-backend-coverage-floor.md`.</li></ul> |
| D18 | **DevOps script-quality obligation** (2026-05-19). <ul><li>Every devops-owned `.ps1` / `.sh` change ships **lint + unit tests + coverage** in the same task.</li><li>Lint: PSScriptAnalyzer (PowerShell) / shellcheck (bash) — zero error-level findings on changed/added scripts.</li><li>Tests: Pester / bats-core, beside scripts under `local/framework.config.yaml § devops-scripts.tests-path`.</li><li>Coverage: line coverage on **changed + added** line set ≥ `devops-scripts.coverage-threshold` (framework default `90`); `bashcov` or `kcov` for bash (adopter picks via `coverage-tool-bash`); native for Pester.</li><li>Authorship boundary moved from `qa-engineer` → `devops-engineer` for files in the devops tree; QA keeps seed / cleanup / smoke / scenario-harness.</li><li>Data-only files exempt; failed gate = stoppable intermediate state, not a follow-up. No-tooling → discovery gap.</li><li>Closes [#28](https://github.com/kostiantyn-matsebora/ginee/issues/28) + sub-issue [#30](https://github.com/kostiantyn-matsebora/ginee/issues/30). Migration: `core/MIGRATIONS/D18-devops-script-quality.md`.</li></ul> |
| D22 | **Doc-authoring protocol for adopter docs** (2026-05-19). <ul><li>Promotes `core/process.md § Documentation style` from aspirational → **binding** for adopter outputs (architecture doc, ADRs, CRs, READMEs, runbooks, scenarios, API docs).</li><li>**Three-file topology** (anticipates #37 multiplying load across roles): shape map + mandatory checks live in `core/process.md § Documentation style` (always-loaded); enforcement + attestation in `core/doc-authoring-protocol.md` (load-on-demand at Phase 5); 6 paired bad/good examples in `core/doc-authoring-examples.md` (load on first-time / explicit request).</li><li>**No custom ginee lint.** Enforcement piggybacks on adopter tooling — `team-lead` discovery records markdown / prose linters via existing `builtin:commands` + `builtin:conventions` recipes; roles run `${commands.lint.docs}` at Phase 5 / report-as-done.</li><li>Attestation: one-line entry in phase-report + PR-description Verification log.</li><li>Cross-issue: hard-reject coupling with #37 (classical SA Review) deferred until #37 lands.</li><li>Closes [#39](https://github.com/kostiantyn-matsebora/ginee/issues/39). Migration: `core/MIGRATIONS/D22-doc-authoring-protocol.md`.</li></ul> |
| D21 | **Context-economy enforcement gate** (2026-05-19). Three layers on this repo's PRs — Claude Code hook + git hooks + CI workflow — block threshold-exceeding framework edits unless paired with an `Optimized-By: ai-engineer` trailer commit. Structural lint flags prose paragraphs with > 2 sentences in always-loaded files (D18–D20 regression signature). Thresholds: 25 lines / 1 KB for always-loaded files; 50 lines / 2 KB elsewhere. Waiver = PR label `context-economy:waived` + justification line in body. Closes [#38](https://github.com/kostiantyn-matsebora/ginee/issues/38). Spec: `scripts/context-economy-check.ps1`. Migration: `core/MIGRATIONS/D21-context-economy-gates.md`. |
| D20 | **Automatic mode — post-PR CI watch** (2026-05-19). <ul><li>Auto mode + Mode 1: after `gh pr create`, enter CI-watch state and run an **iterate-fix-recheck loop** until all required checks are green or a forced-handback trigger fires.</li><li>Default policy **`poll`** (synchronous polling); alternatives `async` / `hybrid` / `disabled` via `local/framework.config.yaml § automatic-mode.ci-watch-policy`.</li><li>**Failure classification** — attributable (`paths:` overlap / log cites changeset file / changeset test name) → Phase 6 + push fix + re-watch; flake (regex corpus, adopter-extensible) → one auto-rerun; unattributable → forced handback.</li><li>**Forced handback** — unattributable, same-check-fails-twice, flake-recurs, timeout, max-cycles (default `3`), user-interrupt, budget-ceiling.</li><li>**Notifications** — ≤3 PR comments per cycle (`"CI watch started"` / `"CI fix pushed (cycle N of M)"` / `"CI complete — all green"`); optional `## CI status` placeholder in PR description.</li><li>**Never auto-merge, never auto-approve, never auto-dismiss reviews, never mask a flake.**</li><li>New `automatic-mode:` config block; affects D12 (Accept extended), D14 (PR-linkage surface), D17 (Mode 1 finalize step 5).</li><li>Closes [#34](https://github.com/kostiantyn-matsebora/ginee/issues/34). Spec: `core/ci-watch.md`. Migration: `core/MIGRATIONS/D20-ci-watch.md`.</li></ul> |
