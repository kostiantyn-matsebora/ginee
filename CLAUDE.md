...
| D40 | **Changelog + release-notes protocol — surface-specific voice + shape** (2026-05-24). Three release surfaces, three voices, three caps — `core/MIGRATIONS/D<N>-*.md` (framework-dev voice; no cap) · `docs/CHANGELOG.md` (verbose record per Keep-a-Changelog; lead-in ≤ 25 words + sub-bullets) · `.github/release-notes/v*.md` (**user-value voice**; **≤ 20 words per bullet**; `(D<N>)` tag suffix). 5 mandatory checks before publishing a sidecar — word cap · user-value voice · D-tag · no implementation boilerplate · migration link in footer. Self-lint by author; same machinery as D22 / D26 / D29 / D30. Closes a recurring drift mode — release-notes sidecars repeatedly drifted into framework-dev voice + oversized bullets (the v0.12.0 4-pass rewrite documented in `core/MIGRATIONS/D40-changelog-protocol.md § Worked example`). Closes [#81](https://github.com/kostiantyn-matsebora/ginee/issues/81). Full: `core/changelog-protocol.md` + `core/MIGRATIONS/D40-changelog-protocol.md`. |
