...
| D31 | **Per-role + per-task model tier** (2026-05-23). Routes reasoning-heavy roles to capable models and execution-heavy roles to cheaper ones. Tier names vendor-neutral in `core/`; model IDs in adapters only. <ul><li>**3 tiers** — `reasoning` · `standard` (default) · `fast`. Claude map: `claude-opus-4-7` / `claude-sonnet-4-6` / `claude-haiku-4-5-20251001`.</li><li>**Defaults** — `team-lead` + `solution-architect` → `reasoning`; `ai-engineer` + 4 engineer cardinals → `standard`. `default-tier:` in each role-kernel frontmatter.</li><li>**Resolution order** — (1) per-task prefix `model:<tier>` (combinable with `auto:` / `branch:` / `wt:` / `commit:`) → (2) Phase-3 answer → (3) `local/framework.config.yaml § model-tier.per-role.<role>` → (4) `core/roles/<role>.md` frontmatter `default-tier:`.</li><li>**Adapter wiring** — Claude install writes `model: <id>` into each `.claude/agents/<role>.md` frontmatter; Cursor / Copilot CLI / Codex / generic emit one-line install warning (no programmatic per-role selection today; per-task prefix documented as hint).</li><li>**Forward-only**; purely additive — absent `model-tier:` → framework defaults apply silently.</li></ul> Full: PLAN.md § D31 + `core/MIGRATIONS/D31-model-tier.md`. Closes [#76](https://github.com/kostiantyn-matsebora/ginee/issues/76). |
