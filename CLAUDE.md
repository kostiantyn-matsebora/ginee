...
| D39 | **Sub-issue dispatch — cross-session traceability + time-tracking** (2026-05-24). Issue-sourced tasks → one GitHub sub-issue per cardinal dispatch under parent; labels `ginee:role:*` + `ginee:phase:*` + inherited `value:*`/`complexity:*`; body per `core/templates/sub-issue-dispatch.md`. Cardinal posts progress comments carrying `time:` + `cumulative:`; D29 phase-report return = closing comment with mandatory `## Time spent`. Stop-state → progress comment; sub-issue stays open. Parent sticky `<!-- ginee:dispatch-map -->` aggregates per-cardinal rollup. **Assignee precedence** — non-empty human assignee overrules role label; cardinal suspended until cleared. **Opt-out** — `notrack:` task prefix → `ginee:track:off` parent label → `framework.config.yaml § dispatch.tracking` → default `sub-issues` on `github.repo`; TODO / freeform / no-`gh` → in-context. Cross-session resume reads parent + open sub-issues; no transcript replay. Closes [#106](https://github.com/kostiantyn-matsebora/ginee/issues/106). Full: `core/MIGRATIONS/D39-sub-issue-dispatch.md`. |
