...
| D22 | **Doc-authoring protocol for adopter docs** (2026-05-19). <ul><li>Promotes `core/process.md § Documentation style` from aspirational → **binding** for adopter outputs (architecture doc, ADRs, CRs, READMEs, runbooks, scenarios, API docs).</li><li>Spec: `core/doc-authoring-protocol.md` — default-shape map + mandatory checks + 6 paired examples (component inventory / design properties / ADR rationale / runbook / API table / scenario).</li><li>Per-role hook: one Source-of-truth row in every cardinal kernel pointing at the protocol.</li><li>**No custom ginee lint.** Enforcement piggybacks on adopter tooling — `team-lead` discovery records markdown / prose linters via existing `builtin:commands` + `builtin:conventions` recipes; roles run `${commands.lint.docs}` at Phase 5 / report-as-done.</li><li>Attestation: one-line entry in phase-report Verification log + PR-description Verification log.</li><li>Cross-issue: hard-reject coupling with #37 (classical SA Review) deferred until #37 lands.</li><li>Closes [#39](https://github.com/kostiantyn-matsebora/ginee/issues/39). Migration: `core/MIGRATIONS/D22-doc-authoring-protocol.md`.</li></ul> |
