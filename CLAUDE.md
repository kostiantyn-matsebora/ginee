...
| D32 | **Claude adapter — accept-orchestrated subagent dispatch** (2026-05-23). Claude Code's `Agent` / `Task` tool is top-level only; subagents cannot fan out. D28's hand-back rule (skill-runner → `@team-lead` → specialists) silently degrades there. D32 narrows the surface boundary *on the Claude adapter only*: team-lead authors the dispatch plan (re-invoked each cycle) · user approves · skill-runner executes the approved contract verbatim · skill-runner passes returns through · team-lead synthesizes + decides next. Decision authority unchanged — team-lead still owns plan / synthesis / next-decision / gate text / routing / defaults. Skill-runner gains *execution* of approved contracts, never origination. Other adapters keep the original D28 rule. Purely additive — no `local/` change, no installer change. Full: `core/MIGRATIONS/D32-claude-adapter-subagent-dispatch.md` + `adapters/claude/install.md § Subagent dispatch limitation (D32)`. Closes [#87](https://github.com/kostiantyn-matsebora/ginee/issues/87). |
