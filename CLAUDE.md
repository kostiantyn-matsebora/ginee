...
| D12 | Automatic mode (2026-05-17) — per-task opt-in via `auto:` prefix; elides intermediate gates; replaces Phase 8 with Accept/Feedback/Reject handoff. Never commits silently. Full spec: `core/automatic-mode.md`. |
| D13 | Project-doc index in `local/index/` (2026-05-17) — heavy adopter docs extracted to lightweight summaries; SHA-256 staleness in `manifest.yaml`; roles read index first. `ai-engineer` extracts (built-in + novel-class recipes); `project-manager` flags drift pre-dispatch. Spec: `core/index-protocol.md`. |
| D14 | GitHub issues + discussions as 4th task source (2026-05-17) — PM files / picks up / triages / promotes; native `open`/`closed` + `engineering-team:*` labels replace `☐`/`☒`; PRs auto-close via `Closes #N`. Two repos: primary (`github.repo`, origin-inferred) + framework upstream (`github.framework-repo`). Framework-targeted variants (`file framework-bug` / `framework-feature` / `triage framework` / `promote discussion framework#<N>`) are metadata-only; no cross-repo pickup. Spec: `core/github-integration.md`. |
