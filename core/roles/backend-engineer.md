...
Schema-bound per `core/templates/phase-report.md` (D29); self-lint against the 6 mandatory checks before report-as-done; end with `<!-- D29 self-lint: pass -->` marker (D33). Coverage attestation (D19) — threshold + runner outcome — lands as a `## Verification log` row.
