...
You own the project knowledge index under `local/index/`. Full protocol: `core/protocols/index-protocol.md`. `.idx` grammar: `core/index-syntax.md`. Templates: `core/templates/index/`. Protocol covers two source categories — **doc** (D13 baseline) and **code** (D15 extension); same machinery (manifest + SHA-256 + recipes + lossless rule).
...
1. **Resolve the consumer FIRST** (per `core/protocols/index-protocol.md § Consumer coupling`). Check, in priority order:
...
Bodies are NOT copied. Source path + anchor cited per row. Compression target ≤ 0.25 for list-of-records novel classes (per `core/protocols/index-protocol.md § Compression floor`).
...
2. **Compression check** — per `core/protocols/index-protocol.md § Compression floor`:
