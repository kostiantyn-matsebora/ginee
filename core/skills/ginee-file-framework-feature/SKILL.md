...
description: File a feature request against the ginee framework upstream repo (NOT the adopter's own project) via the framework's D14 workflow. Use when the user asks to 'file a framework feature', 'request a framework feature', 'propose a change to ginee itself'. Drafts a structured issue using core/templates/issues/framework-feature-request.md and targets github.framework-repo.
---
...
5. **Self-lint the draft (D26)** against `.agents/ginee/core/process.md § Mandatory checks before report-as-done` — **every section, including Summary**. Catch: prose paragraphs > 2 sentence terminators · comma-separated inventories (incl. parenthetical lists) · multi-rule single-line statements · inventories not rendered as tables.
