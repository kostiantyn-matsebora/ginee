...
Thin wrapper. Loads `.agents/ginee/core/protocols/github-integration.md § Review-comment ingestion` for the shared 7-step procedure. The `@team-lead address-review #<PR>` command runs the identical procedure for adapters without AgentSkills.
...
`<N>` = PR number in primary repo per `.agents/ginee/core/protocols/github-integration.md § Repo discovery`. **No `framework-` variant** — checked-out branch must be the PR's head ref.

## Procedure

1. Load `.agents/ginee/core/protocols/github-integration.md § Review-comment ingestion`.
...
4. Run the shared 7-step procedure verbatim under team-lead. Key invariants:
   - Plan-table approval gate is **forced-interactive** — applies even in `auto:` mode per `.agents/ginee/core/protocols/automatic-mode.md § Forced-interactive triggers`.
...
- Never auto-detect new review comments — invocation is explicit; CI-watch loop (`core/protocols/ci-watch.md`) unaffected.
