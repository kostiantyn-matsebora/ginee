...
| Peer-resolved exchange | Single-cardinal defect within in-flight phase → owning engineer resumes via adapter-native peer-resume; team-lead skipped. Escalate on contradictory return, multi-cardinal surface, or scope change. Full taxonomy: `core/protocols/cross-agent-handoff.md § Peer-resolved vs team-lead-escalated exchanges` · round-trip cap: `core/protocols/iteration-protocol.md § Peer round-trip pattern` · vendor-neutral primitive: `core/protocols/adapter-wrapper-pattern.md`. |
