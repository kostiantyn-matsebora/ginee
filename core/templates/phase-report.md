...
- **Doc-style protocol** *(any markdown authored / edited — per `core/doc-authoring-protocol.md`)* — `<discovered linter command>: PASS / N findings (see <path>)` OR `no linter configured; self-checked against § Mandatory checks`.
