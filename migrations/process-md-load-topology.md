...
Adopt-existing-solution candidate (per D30-adopt-existing-solution): the framework's **own load-on-demand extraction pattern**, already proven via `core/protocols/automatic-mode.md` · `core/protocols/options-protocol.md` · `core/protocols/doc-authoring-protocol.md` · `core/protocols/delivery-modes.md` · `core/protocols/index-protocol.md`. D35 applies the same machinery at finer granularity (per-phase) inside `core/process/`. No external library / framework adopted — pattern adoption only.
