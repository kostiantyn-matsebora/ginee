...
2. **Dispatch-contract wiring.**
   - `core/triage-scoring.md § Auto-estimation on pickup` — explicit clause: *"solution-architect gathers signals from `issue body + index entries` only; raw source reads require a one-line justification in the return per `core/templates/phase-report.md § Source reads (this dispatch)`."*
...
| `core/triage-scoring.md` | +2 / -1 — new step in `§ Auto-estimation on pickup` (index-first clause) |
