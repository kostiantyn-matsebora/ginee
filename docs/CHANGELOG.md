...
- **Adapter wrapper-anchor pattern + peer SendMessage between cardinals** ([#189](https://github.com/kostiantyn-matsebora/ginee/issues/189), [#195](https://github.com/kostiantyn-matsebora/ginee/pull/195)). Two interlocking parts close the implicit-pattern gap in the adapter compliance surface.
  - **Wrapper-anchor formalised** — new `core/protocols/adapter-wrapper-pattern.md` names the per-adapter subagent file (`.claude/agents/<role>.md` and siblings) as a *constraint-bearing anchor* whose frontmatter binds host-enforceable properties mechanically. Design invariant: hardenable properties (`tools:` · `model:` · `description:` · `phase-participation:`) MUST live in frontmatter; charter prose MUST NOT restate the binding. Per-adapter availability matrix (Claude / Cursor / Copilot / Codex / generic) + charter-vs-frontmatter audit method ship in the new protocol. Canonical Tier-1 / Class-A entry of the adapter compliance playbook (#135).
  - **Peer SendMessage on specialist wrappers** — `adapters/_shared/agents/{qa,frontend,backend,devops,ai}-engineer.md` gain `SendMessage` in `tools:` so cardinals can resume each other directly within an in-flight phase. SA wrapper unchanged (REJECT/APPROVE-only per HARD CONSTRAINT 2). team-lead already carried `SendMessage`.
  - **Routing taxonomy + round-trip cap** — `core/protocols/cross-agent-handoff.md` gains the peer-resolved-vs-team-lead-escalated 6-row taxonomy + `peer-exchange:` audit-trail field on `## Decisions made`. `core/protocols/iteration-protocol.md` gains the peer round-trip pattern (default cap 3 round-trips per phase before forced team-lead re-entry). `core/process/dispatch.md § Dispatch & parallelism rules` gains the `Peer-resolved exchange` row. `CLAUDE.md § Framework authoring — context economy` adapter-binding bullet gains a one-line callback to the new protocol.
...
- **QA defect-reproducer test — Phase 5 → Phase 6 contract** ([#184](https://github.com/kostiantyn-matsebora/ginee/issues/184)). Every defect routed to Phase 6 now carries a committed failing test that reproduces it; the engineer's fix-oracle is "test passes locally". Collapses the round-trip waste from "works on my machine" reproducer drift, makes the fix-verification deterministic before signal-back, and leaves a permanent regression gate per defect class. Untestable defects (pure visual judgement · human-in-the-loop · timing-sensitive observations) flag `testable: false` + rationale and fall back to description-only Phase 6 dispatch; team-lead surfaces a process-concern advisory when the `testable: false` ratio exceeds `qa.defect-reproducer.testable-false-threshold` (default 20%). Surfaces touched: `core/process/phase-5-testing.md § Defect-reproducer` + `Acceptance` · `core/process/phase-6-bug-fixing.md § Reproducer-test dispatch contract` + `Acceptance` · `core/roles/qa-engineer.md § Defect-reproducer authoring discipline` · `core/roles/qa-engineer.details.md § Test-layer selection per defect class` · `core/templates/phase-report.md § Defects` · `core/templates/sub-issue-dispatch.md § Reproducer test`. Migration: [`migrations/defect-reproducer-contract.md`](https://github.com/kostiantyn-matsebora/ginee/blob/main/migrations/defect-reproducer-contract.md). Soft cutover — existing Phase 6 dispatches without reproducers tolerated as legacy until adopters re-fetch `core/`.
...
- **`/ginee-iterate` skill — relay review-cycle reply to warm cardinal** ([#154](https://github.com/kostiantyn-matsebora/ginee/issues/154), [#187](https://github.com/kostiantyn-matsebora/ginee/pull/187)). New AgentSkill so review-cycle replies on a live cardinal task forward verbatim to the warm agent via `SendMessage` instead of the skill-runner editing from the main thread. Sibling tactic to warm-cardinal-default (T11); preserves warm-reuse savings across iteration cycles that previously short-circuited the cardinal and bloated main-thread context by 50–100k tokens over 10–20 cycles. Skill count 12 → 13. Migration: [`migrations/ginee-iterate-skill.md`](https://github.com/kostiantyn-matsebora/ginee/blob/main/migrations/ginee-iterate-skill.md).
...
- **`solution-architect` realigned to classical-architect boundaries** ([#182](https://github.com/kostiantyn-matsebora/ginee/issues/182)). Two sibling rules close the documented gap that let SA accrete implementation content in architecture artefacts.
  - **Timing — SA out of implementation phases.** `phase-participation: [1, 2, 7]` (down from `[1, 2, 4, 5, 6, 7]`). Phase 4/5/6 SA dispatch categorically refused — no governance dip, no review on engineer mid-flight proposal, no NFR-oracle dip, no architectural-fix dip. Engineer-surfaced architectural delta routes through team-lead's new `§ Engineer-surfaced architectural-delta gate` — outcomes are *defer to next design cycle* OR *stop current task and re-enter Phase 1–2*. Out-of-process Review (periodic / drift / explicit user) targets the architecture-of-record, never engineer mid-flight proposals. Heavy-role-bypass SA1/SA2/SA3 tracks retired.
  - **Phase 7 SA review now conditional.** Default = skip. Fires only when (a) the task introduced architectural changes OR (b) SA's Phase-1 phase-report recorded `post-implementation-governance: yes`. Phase-1 SA design dip adds the `post-implementation-governance: yes/no` row to `## Decisions made`.
  - **Content — implementation rendering forbidden in every SA artefact.** New `§ Implementation rendering — out of scope of every SA artefact` in `core/roles/solution-architect.md` enumerates the forbidden categories (adopter function / member identifiers · line-numbered citations · commit SHAs · handler-body snippets · "how to wire it" prescriptions · repeated adopter file paths as architectural basis). New ADR-template inverse-checklist + worked architectural-mechanism vs implementation-rendering pair in `core/roles/solution-architect.details.md`. New SA-artefact content self-lint in `core/templates/phase-report.md` — SA `## Verification log` MUST carry `SA-artefact content self-lint: PASS / <N findings>` per touched artefact.
  - **Claude-adapter enforcement — HARD-FORCE on both axes** (per playbook [#135](https://github.com/kostiantyn-matsebora/ginee/issues/135)). Two new PreToolUse hooks register on adopter `.claude/settings.json` via `core/scripts/sync-claude-settings.{ps1,sh}`:
    - **T14 — `pre-tool-use-task.{ps1,sh}`** (Class A, Task matcher). Blocks `Task(subagent_type=solution-architect)` when the prompt carries a Phase 4/5/6 indicator. Exit 2 + stderr remediation pointing at team-lead's gate.
    - **T15 — `pre-tool-use-sa-artefact.{ps1,sh}`** (Class A, Edit|Write|MultiEdit matcher). Blocks edits to SA-owned paths (per `local/framework.config.yaml § architecture-doc · adr-directory · diagrams-directory` + `local/requirements.md` + `local/asr-utility-tree.md`) when added content carries `<file>:<line>` citations OR commit SHAs in evidence context (`as of <sha>` · `prior to <sha>` · `commit <sha>` · `revision <sha>`).
    - **Class H bookended text** — CLAUDE.md + adapters/claude/CLAUDE-pointer.md hard-constraint list extended from 5 to 7 entries (#3 SA Phase 4/5/6 refusal · #4 SA artefacts no implementation rendering), bookended top + bottom per T9 recency-optimisation.
    - **Class D reuse** — keyword-triggers.yaml `sa-boundary-and-content` injects the spec excerpt on `@solution-architect` / `ADR` / `architecture-doc` / `ASR` mentions; carry-forward-rules.yaml `solution-architect` row extended to bind warm-SA continuations.
    - **Class A-indirect** — `/ginee-self-lint` slash command consumes the new SA-artefact content self-lint check via `phase-report.md`; backstops the soft-force categories (adopter function/member identifiers · handler-body snippets · "how to wire it" prescriptions · repeated adopter file paths) — pattern-matching adopter identifiers is brittle outside controlled vocabularies, so regex hard-force is deferred for those.
    - Per-tactic opt-out: `local/framework.config.yaml § compliance.disabled: [pretooluse-task-hook]` · `[pretooluse-sa-artefact-hook]`. Emergency bypass per call: `SKIP_GINEE_COMPLIANCE=1`. Migration: [`migrations/sa-boundary-tightening.md`](https://github.com/kostiantyn-matsebora/ginee/blob/main/migrations/sa-boundary-tightening.md).
...
- **Tier 3 compliance — T9 / T10 / T11 / T12 / T13** ([#176](https://github.com/kostiantyn-matsebora/ginee/pull/176), parent: [#135](https://github.com/kostiantyn-matsebora/ginee/issues/135)). The last batch of the maximum-force Claude playbook ships the recency-optimisation pass + slash command suite + main-thread permission lockdown + session-resume hook + Optimized-By attestation gate (ask-mode).
  ...
  - **T13** (sister tactic, ask-mode, push-time) — **Optimized-By attestation gate**. PreToolUse Bash hook surfaces Claude Code's native permission prompt when `git push` would push a range carrying `Optimized-By: ai-engineer` trailer in any commit AND no `Agent(subagent_type=ai-engineer)` dispatch is found in the session transcript. Path-agnostic — fires equally on framework-self-dev (`core/**` · `adapters/**` · `extras/**` commits) and adopter pushes (docs / prompts / `local/` content). Closes the loophole where the trailer claim was voluntary-compliance. Introduces a new force-class axis (*consent-required gate*, Class A nuance) alongside the existing binary gates. Spec: [`migrations/optimized-by-attestation.md`](https://github.com/kostiantyn-matsebora/ginee/blob/main/migrations/optimized-by-attestation.md).
  - All five wire automatically via `/ginee-update` (extended `core/scripts/sync-claude-settings.{ps1,sh}` + `install.{ps1,sh}` slash-commands copy step). Bypass per call: `SKIP_GINEE_COMPLIANCE=1`. New opt-out tactic-ids: `slash-commands` · `main-thread-permissions` · `session-start-hook` · `optimized-by-attestation`.
...
- **Tier 2 compliance hooks — T5 / T6 / T7 / T8** ([#173](https://github.com/kostiantyn-matsebora/ginee/pull/173), parent: [#135](https://github.com/kostiantyn-matsebora/ginee/issues/135)). Four new cross-platform hooks complete the playbook's prompt-time / action-time / turn-time enforcement surface on the Claude Code adapter.
  ...
  - **T8** ([#144](https://github.com/kostiantyn-matsebora/ginee/issues/144)) — PreToolUse hook on `SendMessage` blocks warm-cardinal continuations missing the `[carry-forward] Remember: <rule>` leading anchor. Per-cardinal rules in `adapters/claude/hooks/carry-forward-rules.yaml`. First-dispatch `Agent` calls are out of scope. Spec: [`migrations/carry-forward-injection.md`](https://github.com/kostiantyn-matsebora/ginee/blob/main/migrations/carry-forward-injection.md).
  - All four wire automatically via `/ginee-update` (extended `core/scripts/sync-claude-settings.{ps1,sh}`). Bypass per call: `SKIP_GINEE_COMPLIANCE=1`. Opt-out tactic-ids: `user-prompt-submit-hook` · `posttooluse-edit-hook` · `stop-hook` · `pretooluse-send-message-hook`.
