...
| D11 | Public framework name | **`ginee`** (revised 2026-05-18 from `engineering-team`). Tagline: *AI software engineering team. Behaves like a real team, assigned to project, does self-onboarding and ready to work.* Skill prefix `ginee-` (originally codename, now formal name — consistent at all surfaces). Install path `.agents/ginee/` (revised from `.agents/engineering-team/` per D8). |
