...
1. **Copy `AGENTS.md` to the project root.**
   ...
   ```powershell
   Copy-Item engineering-team\adapters\agents-md\AGENTS.md .\AGENTS.md
   ```

   ```bash
   cp engineering-team/adapters/agents-md/AGENTS.md ./AGENTS.md
   ```
...
4. **Verify** — prompt `@solution-architect status` (or "act as solution-architect and report status"). Confirm it loads the canonical charter from `engineering-team/core/roles/solution-architect.md` and the project bindings.
...
2. Re-copy `engineering-team/adapters/agents-md/AGENTS.md` to project root (merge if you added project-specific content).
...
- Claude Code → also install `engineering-team/adapters/claude/`
- Copilot CLI → also install `engineering-team/adapters/copilot-cli/`
